...
TracerROSMessenger::TracerROSMessenger(TracerBase *tracer, ros::NodeHandle *nh) : tracer_(tracer), nh_(nh)
{
}
...
void TracerROSMessenger::LightCmdCallback(const tracer_msgs::TracerLightCmd::ConstPtr &msg)
{
    if (!simulated_robot_)
    {
        if (msg->enable_cmd_light_control)
        {
            ...
            {
                cmd.front_mode = TracerLightCmd::LightMode::CONST_OFF;
                break;
            }
            case tracer_msgs::TracerLightCmd::LIGHT_CONST_ON:
            {
                cmd.front_mode = TracerLightCmd::LightMode::CONST_ON;
                break;
            }
            case tracer_msgs::TracerLightCmd::LIGHT_BREATH:
            {
                cmd.front_mode = TracerLightCmd::LightMode::BREATH;
                break;
            }
            case tracer_msgs::TracerLightCmd::LIGHT_CUSTOM:
            {
                cmd.front_mode = TracerLightCmd::LightMode::CUSTOM;
            ...
            case tracer_msgs::TracerLightCmd::LIGHT_CONST_OFF:
            {
                cmd.rear_mode = TracerLightCmd::LightMode::CONST_OFF;
                break;
            }
            case tracer_msgs::TracerLightCmd::LIGHT_CONST_ON:
            {
                cmd.rear_mode = TracerLightCmd::LightMode::CONST_ON;
                break;
            }
            case tracer_msgs::TracerLightCmd::LIGHT_BREATH:
            {
                cmd.rear_mode = TracerLightCmd::LightMode::BREATH;
                break;
            }
            case tracer_msgs::TracerLightCmd::LIGHT_CUSTOM:
            {
                cmd.rear_mode = TracerLightCmd::LightMode::CUSTOM;
                cmd.rear_custom_value = msg->rear_custom_value;
                break;
            }
            }

            tracer_->SetLightCommand(cmd);
        }
        else
        {
...
void TracerROSMessenger::PublishStateToROS()
{
    ...
    auto state = tracer_->GetTracerState();
