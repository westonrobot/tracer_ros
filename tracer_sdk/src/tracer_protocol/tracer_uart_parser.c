...
#include "tracer_sdk/tracer_protocol/tracer_uart_parser.h"
...
typedef union {
    ...
    TracerControlMessage control_msg;
} TracerDecodedMessage;
...
static bool ParseChar(uint8_t c, TracerDecodedMessage *msg);
static uint8_t CalcBufferedFrameChecksum();
static bool ConstructStatusMessage(TracerStatusMessage *msg);
static bool ConstructControlMessage(TracerControlMessage *msg);
...
{
    static TracerDecodedMessage decoded_msg;
...
bool ParseChar(uint8_t c, TracerDecodedMessage *msg)
{
...
bool ConstructControlMessage(TracerControlMessage *msg)
{
    if (msg == NULL)
        return false;
...
bool ConstructStatusMessage(TracerStatusMessage *msg)
{
    if (msg == NULL)
        return false;
